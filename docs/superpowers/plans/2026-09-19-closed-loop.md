...
- [ ] **Step 1b: Restore the replay fixtures' natural order.** Task 5's `test_the_769_sequence_repairs_five_times_and_merges` and `test_three_identical_fails_park_for_no_progress` (in `v2/tests/coordinator/test_step.py`) record each FAIL round as verdict → `start_implementation` → `request_repair` → `start_implementation`, because until this task a `request_revision` landed at `planned`. Now that it lands at `reviewing`, remove the first `start_implementation` of each round and the comment that explains it, so the fixtures replay the loop's own order: verdict → `request_repair` → `start_implementation`. Assertions unchanged. Run `tests/coordinator/test_step.py`.
