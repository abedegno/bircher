...
A caller-presented value the kernel refuses unless it equals a value the kernel holds counts as **observed**: the caller can only ever supply the right answer, and the value that survives is the kernel's. **Twenty-five rows are asserted after Milestone 1**, and the count is stated here as a number so it can be checked: verdict, CI status, CI head, requested head, context bundle hash, PR, repo, policy version, the front half's park reason and cursor item, the park's session and reviewer (the coordinator's own reading of a listing, bound by the §8 cursor invariant and the §8 proof, not by a kernel object — the turn's OWN `session` is bound to a kernel object as of Task 10 and moved to observed), the human's question ids, answer, direction text and review findings, the model's question id, question, ruling, reasoning and cost_if_wrong, the recorded prompt item's item_id, and the front half's own three: the policy inputs fetched by the runner adapter rather than the kernel, the stopped session's teardown, and the runner's own resumption lock. Fifteen are residuals to close; the remaining ten — the verdict, the human's four, and the model's five — are asserted permanently and by intent: a verdict *is* the reviewer's judgment, a human's words are the human's to say, and the model's questions and rulings are what it asked and decided; the kernel binds who gave them and when, never whether they are right or what they should have been. `test_the_spec_and_the_table_agree_on_the_count` fails if this sentence and the table disagree, because a number in prose that nothing checks is the defect this document is about.
