...
**Availability, checked rather than assumed.** The proxy landed in upstream #236 and is an ancestor of `v0.7.0` — the version the NAS runner already runs — so this needs neither an upstream contribution nor a runner bump. `bubblewrap` is installed in the runner image (`deploy/docker/Dockerfile:213`). Implementation spans `omnigent/inner/credential_proxy.py`, the egress controller and proxy, all three sandbox backends and the spec loader, with unit tests and a sandbox e2e suite.

**The one unverified item** is whether `linux_bwrap` can actually start inside the runner container on this host: bwrap needs unprivileged user namespaces, and a container runtime's seccomp or apparmor profile can deny them. That is a single empirical check — start a session with a trivial sandbox block and see whether it runs or reports bwrap unavailable — and Milestone 1 runs it before anything depends on the result. It is a deployment configuration question with known remedies, not an open design question.

**The cross-domain handoff no longer needs an exotic channel.** The earlier revision demanded a container-manager copy-out or an append-only upload service, on the grounds that a shared workspace path is exactly what requirement 5 forbids. Content addressing removes the need. Kernel-owned git storage is sited **outside every `write_path`** — unwritable from the sandbox, and maskable via `mask_paths` — and the kernel imports by fetching from the model's worktree into that storage, resolving the commit SHA there, verifying reachability, and pushing **the SHA it resolved after import**. A late write in the model's worktree produces different objects with different hashes, which the kernel never fetched and will never push. Requirement 5's late-writer concern is a TOCTOU on a mutable reference; naming the object by its content is the standard answer to it, and requirement 4 already committed to it.
