...
**The property everything rests on is that the proxy is the only egress path.** Not that traffic is *configured* to prefer it — that the sandbox has no other route. omnigent achieves this under `linux_bwrap` by combining `--unshare-net` (the helper gets no network stack at all) with a TCP-to-Unix relay reaching the parent's proxy through a **bind-mounted Unix socket** (`bwrap_sandbox.py:569`, `:615`). A client that ignores `HTTP_PROXY` has no network to fall back on. Any substitute backend must reproduce *that* property, not merely filter traffic.
