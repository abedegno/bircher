...
A caller-presented value the kernel refuses unless it equals a value the kernel holds counts as **observed**: the caller can only ever supply the right answer, and the value that survives is the kernel's. Four inputs remain genuinely asserted after Milestone 1 — CI status, CI head, the context bundle hash and the policy version — and they are named as residuals rather than left to be mistaken for checks. The verdict itself is asserted permanently and by intent: a verdict *is* the reviewer's judgment, and the kernel binds who gave it and what it was about, not whether it is right.
