...
The scoped-credential option is superseded rather than retained. It required "a real enforcement mechanism — an isolated staging repository or fork, or a ref-enforcing proxy." The third of those exists upstream, is deployed, and is stronger than the property this section was trying to buy.

### The enforcement mechanism exists upstream and is already deployed
...
**Availability, checked rather than assumed.** The proxy landed in upstream #236 and is an ancestor of `v0.7.0`, so it is present in that release and every one since — containment is monotonic, which is what makes the deployment question cheap. Implementation spans `omnigent/inner/credential_proxy.py`, the egress controller and proxy, all three sandbox backends and the spec loader, with unit tests and a sandbox e2e suite. `bubblewrap` is installed in the runner image definition (`deploy/docker/Dockerfile:213`).

An earlier draft of this section named `v0.7.0` as the deployed version. That was stale: Bircher's own `#60` fix replaced an endpoint **omnigent v0.9.0 removed** (`batch/run-queue.sh:1066`), so the runner is at least v0.9.0. The conclusion survives the correction — any version at or above v0.7.0 carries the proxy — but the reasoning must not: a repository tag is not deployment evidence. The runtime check below verifies the running container, not a tag.

**The unverified items, and they are runtime facts rather than design questions.** Milestone 1 establishes all three before anything depends on them: the running container's image digest and omnigent version (at or above v0.7.0); whether `linux_bwrap` can actually start there, since bwrap needs unprivileged user namespaces and a container runtime's seccomp or apparmor profile can deny them; and the production `egress_rules` themselves, exercised rather than read. Each has known remedies.

**The cross-domain handoff no longer needs an exotic channel.** The earlier revision demanded a container-manager copy-out or an append-only upload service, on the grounds that a shared workspace path is exactly what requirement 5 forbids. Content addressing removes the need. Kernel-owned git storage is sited **outside every `write_path`** — unwritable from the sandbox, and maskable via `mask_paths` — and the kernel imports by fetching from the model's worktree into that storage, resolving the commit SHA there, verifying reachability, and pushing **the SHA it resolved after import**. A late write in the model's worktree produces different objects with different hashes, which the kernel never fetched and will never push. Requirement 5's late-writer concern is a TOCTOU on a mutable reference; naming the object by its content is the standard answer to it, and requirement 4 already committed to it. The condition this rests on is exact and belongs in the plan: **after import the kernel uses only the object ID it resolved in kernel-owned storage — never the source ref, branch or path** — and that ID is bound to the attempt generation and review tuple, under the same generation requirement that already governs accepted results and effects. Fetching from a mutable worktree decides *which* content is imported; it cannot alter content already selected by verified SHA.

So this is not a planning blocker. Milestone 1 is implementable in this repository, against a deployed capability, and the end-to-end capability test in the acceptance criteria is what proves the configuration correct.
