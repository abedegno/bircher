...
A caller-presented value the kernel refuses unless it equals a value the kernel holds counts as **observed**: the caller can only ever supply the right answer, and the value that survives is the kernel's. **Seventeen rows are asserted after Milestone 1**, and the count is stated here as a number so it can be checked: verdict, CI status, CI head, requested head, context bundle hash, PR, repo, policy version, the front half's park reason and cursor item, the turn's session together with the park's session and reviewer (all three shape-checked until Tasks 10 and 12 bind them), and the human's question ids, answer, direction text and review findings. Twelve are residuals to close; the remaining five — the verdict, and the human's four — are asserted permanently and by intent: a verdict *is* the reviewer's judgment, and a human's words are the human's to say; the kernel binds who gave them and when, never whether they are right or what they should have been. `test_the_spec_and_the_table_agree_on_the_count` fails if this sentence and the table disagree, because a number in prose that nothing checks is the defect this document is about.
