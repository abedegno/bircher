...
| 2591 | `gh pr close` | `pull_request` |
| 2983 | `gh issue comment` (park-notice) | `comment` |
| 4054 | `gh issue comment` | `comment` |
| 4055 | `gh issue edit --remove-label` | `issue_or_label` |
| 4056 | `gh issue edit --add-label` | `issue_or_label` |
| 4072 | `gh issue close` | `issue_or_label` |
| 4682 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7600 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8516 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8517 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8540 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
