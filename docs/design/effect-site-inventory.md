...
| 2642 | `gh pr close` | `pull_request` |
| 3880 | `gh issue comment` | `comment` |
| 3881 | `gh issue edit --remove-label` | `issue_or_label` |
| 3882 | `gh issue edit --add-label` | `issue_or_label` |
| 3898 | `gh issue close` | `issue_or_label` |
| 4479 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7309 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8251 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8252 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8275 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
