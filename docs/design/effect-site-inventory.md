...
| 2932 | `gh issue comment` (park-notice) | `comment` |
| 4135 | `gh issue comment` | `comment` |
| 4136 | `gh issue edit --remove-label` | `issue_or_label` |
| 4137 | `gh issue edit --add-label` | `issue_or_label` |
| 4153 | `gh issue close` | `issue_or_label` |
| 4263 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7469 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8385 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8386 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8409 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
