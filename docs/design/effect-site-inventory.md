...
| 5097 | `gh api repos/$REPO/statuses/$sha -X POST` | `status_check` |
...
| 5109 | `gh pr comment` | `comment` |
| 3711 | `gh issue comment` | `comment` |
| 3712 | `gh issue edit --remove-label` | `issue_or_label` |
| 3713 | `gh issue edit --add-label` | `issue_or_label` |
| 3730 | `gh issue close` | `issue_or_label` |
| 4021 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 6712 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 7629 | `_contains "$_body" '_net_run … git push origin'` | selftest asserting the source contains it |
| 7630 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 7653 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
