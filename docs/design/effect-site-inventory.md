...
| 4132 | `gh issue comment` | `comment` |
| 4133 | `gh issue edit --remove-label` | `issue_or_label` |
| 4134 | `gh issue edit --add-label` | `issue_or_label` |
| 4150 | `gh issue close` | `issue_or_label` |
| 4260 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7452 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8368 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8369 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8392 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
