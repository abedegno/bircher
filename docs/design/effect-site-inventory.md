...
| 5009 | `gh api repos/$REPO/statuses/$sha -X POST` | `status_check` |
...
| 5021 | `gh pr comment` | `comment` |
| 3687 | `gh issue comment` | `comment` |
| 3688 | `gh issue edit --remove-label` | `issue_or_label` |
| 3689 | `gh issue edit --add-label` | `issue_or_label` |
| 3705 | `gh issue close` | `issue_or_label` |
| 3940 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 6543 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 7460 | `_contains "$_body" '_net_run … git push origin'` | selftest asserting the source contains it |
| 7461 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 7484 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
