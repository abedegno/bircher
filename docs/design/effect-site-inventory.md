...
| 4077 | `gh issue comment` | `comment` |
| 4078 | `gh issue edit --remove-label` | `issue_or_label` |
| 4079 | `gh issue edit --add-label` | `issue_or_label` |
| 4095 | `gh issue close` | `issue_or_label` |
| 4754 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7296 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8212 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8213 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8236 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
