...
| 5104 | `gh api repos/$REPO/statuses/$sha -X POST` | `status_check` |
...
| 5116 | `gh pr comment` | `comment` |
| 3718 | `gh issue comment` | `comment` |
| 3719 | `gh issue edit --remove-label` | `issue_or_label` |
| 3720 | `gh issue edit --add-label` | `issue_or_label` |
| 3737 | `gh issue close` | `issue_or_label` |
| 4028 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 1107 | `curl -X POST $SERVER/v1/sessions/$1/events` | `session_control` |
...
| 6719 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 7636 | `_contains "$_body" '_net_run … git push origin'` | selftest asserting the source contains it |
| 7637 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 7660 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
