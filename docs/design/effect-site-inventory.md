...
| 3830 | `gh issue comment` | `comment` |
| 3831 | `gh issue edit --remove-label` | `issue_or_label` |
| 3832 | `gh issue edit --add-label` | `issue_or_label` |
| 3848 | `gh issue close` | `issue_or_label` |
| 4458 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7366 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8282 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8283 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8306 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
