...
| 2599 | `gh pr close` | `pull_request` |
| 3837 | `gh issue comment` | `comment` |
| 3838 | `gh issue edit --remove-label` | `issue_or_label` |
| 3839 | `gh issue edit --add-label` | `issue_or_label` |
| 3855 | `gh issue close` | `issue_or_label` |
| 4464 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7313 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8219 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8220 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8243 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
