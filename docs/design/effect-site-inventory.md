...
| 4061 | `gh issue comment` | `comment` |
| 4062 | `gh issue edit --remove-label` | `issue_or_label` |
| 4063 | `gh issue edit --add-label` | `issue_or_label` |
| 4079 | `gh issue close` | `issue_or_label` |
| 4689 | `gh issue edit --add-label bircher:running` | `issue_or_label` |
...
| 7607 | `[ "$MERGE_NOTE" = "merge deferred: gh pr merge failed" ]` | string comparison |
| 8523 | `_contains "$_body" '_effect ref_update … git push origin'` | selftest asserting the source contains it |
| 8524 | `echo "FAIL #62: the recovery git push must be routed AND bounded"` | failure message |
| 8547 | `echo "FAIL #62: … a git push that ignores SIGTERM …"` | failure message |
...
| 3834 | `gh run rerun` | *(none)* | Same site class as the other `gh run rerun` row above — see it for the reasoning. **Named, not numbered**, because the repointing tool remaps the Line column and cannot follow a line number written in prose: this reference had been stale since the row moved to 3457. |
