...
| 17 | ~~a one-piece shaping ruling has no gate, and the spec phase cannot hand a run back to `shaping` — muesli #12 was ruled a design document and built as a forty-file implementation (live log E9, finding 20)~~ | **CLOSED** | `request_reshape` and `approve_one_piece` — shaping spec revision 16 (2026-09-12), which is the authority for the mechanism; proven by `assert_dispute`, the proof's fifth §6 line | — |
| 18 | the front-half proof itself carries five gaps, found by the revision-16 review and predating revision 16 — code this branch does not touch: a `review_verdict` missing its own `ruling` key escapes `assert_journal`'s brief-rerender loop and `front.newest_review_verdict` at once; `newest_review_verdict`'s newest-wins read hides a middle acceptance recorded at the same generation; `main`'s `kids[-1]` proves only the newest run per child issue; `accepted_slices_hash`'s unbounded newest-wins read resolves two conflicting approvals silently; and a present, correctly-typed hash naming a blob that does not exist still crashes the proof rather than reporting a line | design | not revision 16's to fix — each gap's forged-history reproduction is in `task-10-rereview-3.md` | a proof-hardening pass, scoped separately |
