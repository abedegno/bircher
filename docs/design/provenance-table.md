...
| `cmd.payload['pr']` | `request_merge`, `record_ci_observation` | asserted | **Residual, blocked on PR creation.** The requester names the pull request. The kernel now binds the merge EFFECT to this recorded target, so the effect cannot diverge from the authorization — but the link from *this run's artifact* to *that PR number* is never observed. Closing it requires the kernel to create the PR itself and record the number, which does not exist yet (round 6, C8). At `record_ci_observation` (closed-loop spec §3) it is shape-checked to a string of digits, no more; the same residual applies. |
...
| `cmd.payload['reason']` | `park` | asserted | **Residual, §4.** The coordinator's reading of a park and of a listing: `reason` is bounded to `PARK_REASONS`, `cursor_item_id` to the §4 cursor invariant test (Task 19), neither to a kernel object. `disagreement` (revision 16) is additionally bounded to a run whose dispute is unresolved (`front.unresolved_disagreement`, `_check_park`): the park's acceptance depends on journal state beyond its own from-set, unlike the other six reasons, which `PARK_REASONS` membership alone admits. |
| `cmd.payload['cursor_item_id']` | `park`, `record_human_answer`, `record_human_direction`, `dismiss_human_item`, `approve_artifact`, `grant_round`, `approve_one_piece`, the human's `record_review` | asserted | **Residual, §4.** The coordinator's reading of a listing, carried on every fact it records from one -- the three ruling shapes included, because a ruling that transitions nothing (`grant_round`) would otherwise leave the message it was read from ahead of the cursor and the next listing would read it again. One shape check (`_check_cursor`: a string or null) for all of them; the VALUE is bounded by the §4 cursor invariant test (Task 19), not by a kernel object. `approve_one_piece` (revision 16) is stricter than the rest: the other human commands tolerate this value's absence (`_check_cursor`'s string-or-null), and `approve_one_piece` refuses its absence outright (`isinstance(cmd.payload.get('cursor_item_id'), str)`). |
...
| `cmd.payload['cause']` | `request_repair`, `park` | asserted | **Residual, closed-loop spec §1.** Why the repair is being requested: CI failure, review failure, or plan conformance gap. Bounded to `REPAIR_CAUSES`, not to a kernel object. The runner determines whether a repair is warranted; the kernel records only that one was requested. A `no_progress` park carries the same field and `_check_park` reads it, so the value enters at `park` too -- there it must match what the two preceding repairs carried, which is the one place this input is bound to journal state rather than to shape alone. |
| `cmd.payload['head_sha']` | `request_repair` | asserted | **Residual, closed-loop spec §1.** The git HEAD the runner is requesting repair for. Shape-checked to a 40-hex string; binding to the newest CI observation is a declared follow-up (round 10, C10). Until then, the head is asserted by the runner with no kernel verification. |
| `cmd.payload['evidence']` | `request_repair`, `park` | asserted | **Residual, closed-loop spec §1.** Observations supporting the repair request: CI job logs, review findings, or plan conformance analysis. Bounded to a list of strings in shape; the content and sufficiency of evidence are the runner's judgment. The kernel records only that evidence was supplied. A `no_progress` park carries the same field and `_check_park` reads it, so the value enters at `park` too -- there it is compared, as a set, against the two preceding repairs' evidence. |
| `cmd.payload['fingerprints']` | `record_review` | asserted | **Residual, closed-loop spec §3.** The blocking findings' fingerprints: sha1 of the first file path and first sentence, computed by the coordinator from the reviewer's own text and shape-checked as a list of strings. The kernel does not recompute or verify them, only records what the derivation computed, so the no-progress judgement can compare one round's fingerprints against the next. |
| `cmd.payload['pr_state']` | `record_ci_observation` | asserted | **Residual, closed-loop spec §1, §3.** The PR's state -- open, closed or merged -- as an external actor's reading of GitHub; shape-checked against that vocabulary, not verified against a kernel object. The kernel does not query GitHub itself, so a PR a person closed reaches the journal only because the runner said so. |
| `cmd.payload['failing_jobs']` | `record_ci_observation` | asserted | **Residual, closed-loop spec §3.** The failing blocking checks' names, an external actor's reading of `gh pr checks`; shape-checked as a list of strings, not verified against a kernel object. A `ci_red` repair's evidence, and only as trustworthy as the reading that produced it. |
...
| `cmd.payload['reasoning']` | `record_model_ruling`, `record_one_piece`, `request_reshape` | asserted | **Intentional and permanent.** The model's words. For `record_model_ruling`, the kernel binds the epoch and phase the question was asked in and refuses a ruling on a question no `model_question` of the epoch asked. `record_one_piece` and `request_reshape` (revision 16) both concern the reserved shape question, which no `model_question` ever asks -- `record_model_question`/`record_model_ruling` refuse `question_id == SHAPE_QUESTION` outright -- so that binding does not cover either of them; this was a revision-15 omission for `record_one_piece`, corrected here. Each refuses only an empty `reasoning` (`_non_empty_str`), asserted, rendered to the next seat and to the person, and verified by nobody. `record_one_piece` additionally refuses a second ruling in the same visit (one decision per visit, shaping spec §2); `request_reshape` additionally refuses when the epoch holds no shape ruling to dispute, or already holds a hand-back (one per bundle, shaping spec §2). |
| `cmd.payload['cost_if_wrong']` | `record_model_ruling`, `record_one_piece` | asserted | **Intentional and permanent.** The model's words. For `record_model_ruling`, the kernel binds the epoch and phase the question was asked in and refuses a ruling on a question no `model_question` of the epoch asked. `record_one_piece` concerns the reserved shape question, which no `model_question` ever asks -- `record_model_question`/`record_model_ruling` refuse `question_id == SHAPE_QUESTION` outright -- so that binding does not cover it; a revision-15 omission, corrected here. It refuses only an empty `cost_if_wrong` (`_non_empty_str`) and a second ruling in the same visit (one decision per visit, shaping spec §2). |
...
| `cmd.payload['detail']` | `record_author_empty` | asserted | **Residual, shaping §3, revision 16.** The coordinator's own reason a hand-back file did not parse, shape-checked (a string or absent) and rendered to the next seat's brief (Task 7) so a malformed `reshape.md` is told apart from a turn that wrote nothing; there is no kernel object to verify the prose against, only the shape. |
