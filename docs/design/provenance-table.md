...
| the session's vendor | the `sess-create` effect's confirmation | observed | The server's own response to `sess-create` names the session's `agent_name`, recorded verbatim as the effect's `external_object_id` when the create is confirmed -- not chosen by the coordinator. `session.py`'s poll compares each read's `agent_id` against that snapshot's, and compares nothing on a read that failed, so the round's session is the one whose vendor the kernel can trust because the server said so, not because an actor claimed it. |
