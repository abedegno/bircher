...
| `cmd.payload['cause']` | `request_repair` | asserted | **Residual, closed-loop spec §1.** Why the repair is being requested: CI failure, review failure, or plan conformance gap. Bounded to `REPAIR_CAUSES`, not to a kernel object. The runner determines whether a repair is warranted; the kernel records only that one was requested. |
...
| `cmd.payload['evidence']` | `request_repair` | asserted | **Residual, closed-loop spec §1.** Observations supporting the repair request: CI job logs, review findings, or plan conformance analysis. Bounded to a list of strings in shape; the content and sufficiency of evidence are the runner's judgment. The kernel records only that evidence was supplied. |
