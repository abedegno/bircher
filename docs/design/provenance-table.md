...
| `cmd.payload['reasoning']` | `record_model_ruling`, `record_one_piece`, `request_reshape` | asserted | **Intentional and permanent.** The model's words; the kernel binds the epoch and phase they were asked in and refuses a ruling on a question no `model_question` of the epoch asked. `request_reshape` (revision 16) is the same row: its `reasoning` is refused unless non-empty (`_non_empty_str`), and it is the same actor's words on the same footing as a ruling's. |
